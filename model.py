...
def get_network_input(player, apple):
    ...
    x = torch.cat([torch.from_numpy(player.pos).float(), torch.from_numpy(apple.pos).float(),
                   torch.from_numpy(player.dir).float(), torch.tensor(proximity).float()])
    return x
